    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var client = new HttpClient())
            {
                ...
                Guid id = Guid.Empty;
                HttpResponseMessage response = client.GetAsync(String.Format("api/device/{0}", hostname)).Result;
                if (response.StatusCode == HttpStatusCode.NotFound)
                {
                    HttpResponseMessage createresp = client.PostAsJsonAsync("api/device/", hostname).Result;
